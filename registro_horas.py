...
if not st.session_state.autenticado:
    ...
    # === NUEVO BLOQUE: Calendario por mes ===
    st.markdown("---")
    st.subheader("📅 Registros mensuales anteriores")
    df_usuario["Mes"] = df_usuario["Fecha"].dt.strftime("%m-%Y")
    meses = df_usuario["Mes"].unique().tolist()

    if meses:
        mes_seleccionado = st.selectbox("Selecciona el mes para ver registros", sorted(meses, reverse=True))
        df_mes = df_usuario[df_usuario["Mes"] == mes_seleccionado].copy()
        df_mes = df_mes.sort_values("Fecha", ascending=False)
        df_mes["Fecha"] = df_mes["Fecha"].dt.strftime("%d/%m/%Y")
        st.dataframe(df_mes[["Fecha", "Tipo de Hora", "Horas", "Centro de Costo", "Comentario"]])

        # Descarga Excel del mes
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            df_mes.to_excel(writer, index=False, sheet_name=f"Registro_{mes_seleccionado}")
        st.download_button(
            label="📥 Descargar Excel del mes",
            data=output.getvalue(),
            file_name=f"registro_horas_{mes_seleccionado}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
    else:
        st.info("🔎 No hay registros mensuales aún.")
