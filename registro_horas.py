...
DB_NAME = "C:/Users/sfarf/OneDrive/PYTHON/registro-horas-bases/registro_horas.db"
        ...
        if uploaded_file:
            try:
                ...
                st.error(f"❌ Error procesando el archivo: {e}")
                    ...
                    # === VISOR DIRECTO DE REGISTROS (SOLO ADMIN) ===
    if usuario == administrador:
        st.markdown("---")
        st.subheader("📋 Visor completo de registros actuales en base de datos")

        try:
            with sqlite3.connect(DB_NAME) as conn:
                df_todos = pd.read_sql_query("SELECT * FROM registros", conn)
                df_todos["fecha"] = pd.to_datetime(df_todos["fecha"]).dt.strftime("%d/%m/%Y")
